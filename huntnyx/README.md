...
```bash
python3 -m huntnyx.cli --check-deps
```

## Installation

```bash
git clone https://github.com/<your-username>/huntnyx.git
cd huntnyx
```

HuntNyx runs as a Python package. Clone it into a directory named `huntnyx` (as above) and run it from the parent directory with `python3 -m huntnyx.cli`.

## Usage

```
python3 -m huntnyx.cli [target] [flags] [options]
```
...
python3 -m huntnyx.cli http://site.thm --all

# Just the Testing stage, authenticated with a session cookie
python3 -m huntnyx.cli http://site.thm/?id=1 --testing --cookie 'session=...'

# Enumeration, writing discovered vhosts to /etc/hosts (root required)
sudo python3 -m huntnyx.cli http://site.thm --enumeration --add-hosts

# Discovery only, skipping nmap by seeding ports
python3 -m huntnyx.cli 10.10.10.5 --ports-list 80,8080 --discovery
